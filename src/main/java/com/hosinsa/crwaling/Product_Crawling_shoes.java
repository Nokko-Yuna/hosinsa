...
public class Product_Crawling_shoes {
	static ArrayList<ProductVO> shoesList = new ArrayList<ProductVO>();
	
	public static void main(String[] args) {
		Product_Crawling_shoes pcraw = new Product_Crawling_shoes();
...
//				System.out.println(img_shoes.size());
				...
				try {
					for(int j=0; j<img_shoes.size(); j++) {
						Elements pNum = doc_shoes.select("div.list-box li.li_box");
						Document doc2 = Jsoup.connect("https://www.musinsa.com/app/goods/"+pNum.get(j).attr("data-goods-no")+"?loc=goods_rank").get();
						Elements pName = doc_shoes.select("div.list-box li.li_box div.article_info p.list_info a");
						Elements pBrand = doc_shoes.select("div.list-box li.li_box div.article_info p.item_title a");
						Elements pPrice = doc2.select("ul.product_article #normal_price");
						Elements imgSmall = doc_shoes.select("div.list-box li.li_box img.lazyload");
						...
						File fileSmall = new File("C:\\Works2\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp1\\wtpwebapps\\Hosinsa\\images\\shoes\\small_"+pNumber+".jpg");
						File fileBig = new File("C:\\Works2\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp1\\wtpwebapps\\Hosinsa\\images\\shoes\\big_"+pNumber+".jpg");						
						...
						proimgSmall = "shoes\\small_"+pNumber+".jpg";
						proimgBig = "shoes\\big_"+pNumber+".jpg";
