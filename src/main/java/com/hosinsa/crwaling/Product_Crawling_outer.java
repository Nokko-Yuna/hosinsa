...
public class Product_Crawling_outer {
	static ArrayList<ProductVO> outerList = new ArrayList<ProductVO>();
	
	public static void main(String[] args) {
		Product_Crawling_outer pcraw = new Product_Crawling_outer();
...
//				System.out.println(img_shoes.size());
				...
				try {
					for(int j=0; j<img_outer.size(); j++) {
						Elements pNum = doc_outer.select("div.list-box li.li_box");
						Document doc2 = Jsoup.connect("https://www.musinsa.com/app/goods/"+pNum.get(j).attr("data-goods-no")+"?loc=goods_rank").get();
						Elements pName = doc_outer.select("div.list-box li.li_box div.article_info p.list_info a");
						Elements pBrand = doc_outer.select("div.list-box li.li_box div.article_info p.item_title a");
						Elements pPrice = doc2.select("ul.product_article #normal_price");
						Elements imgSmall = doc_outer.select("div.list-box li.li_box img.lazyload");
						Elements imgBig = doc2.select("div.wrap_product div.product-img img");
						
						String pCategory = "아우터";
						...
						File fileSmall = new File("C:\\Works2\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp1\\wtpwebapps\\Hosinsa\\images\\outer\\small_"+pNumber+".jpg");
						File fileBig = new File("C:\\Works2\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp1\\wtpwebapps\\Hosinsa\\images\\outer\\big_"+pNumber+".jpg");
						...
						proimgSmall = "outer\\small_"+pNumber+".jpg";
						proimgBig = "outer\\big_"+pNumber+".jpg";
