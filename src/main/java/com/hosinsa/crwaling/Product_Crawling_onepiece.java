...
public class Product_Crawling_onepiece {
	static ArrayList<ProductVO> onepieceList = new ArrayList<ProductVO>();
	
	public static void main(String[] args) {
		Product_Crawling_onepiece pcraw = new Product_Crawling_onepiece();
...
//				System.out.println(img_shoes.size());
				...
				try {
					for(int j=0; j<img_onepiece.size(); j++) {
						Elements pNum = doc_onepiece.select("div.list-box li.li_box");
						Document doc2 = Jsoup.connect("https://www.musinsa.com/app/goods/"+pNum.get(j).attr("data-goods-no")+"?loc=goods_rank").get();
						Elements pName = doc_onepiece.select("div.list-box li.li_box div.article_info p.list_info a");
						Elements pBrand = doc_onepiece.select("div.list-box li.li_box div.article_info p.item_title a");
						Elements pPrice = doc2.select("ul.product_article #normal_price");
						Elements imgSmall = doc_onepiece.select("div.list-box li.li_box img.lazyload");
						Elements imgBig = doc2.select("div.wrap_product div.product-img img");
						
						String pCategory = "원피스";
						...
						File fileSmall = new File("C:\\Works2\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp1\\wtpwebapps\\Hosinsa\\images\\onepiece\\small_"+pNumber+".jpg");
						File fileBig = new File("C:\\Works2\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp1\\wtpwebapps\\Hosinsa\\images\\onepiece\\big_"+pNumber+".jpg");
						...
						proimgSmall = "onepiece\\small_"+pNumber+".jpg";
						proimgBig = "onepiece\\big_"+pNumber+".jpg";
