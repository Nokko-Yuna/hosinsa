...
public class Product_Crawling_skirt {
	static ArrayList<ProductVO> skirtList = new ArrayList<ProductVO>();
	
	public static void main(String[] args) {
		Product_Crawling_skirt pcraw = new Product_Crawling_skirt();
...
//				System.out.println(img_shoes.size());
				...
				try {
					for(int j=0; j<img_skirt.size(); j++) {
						Elements pNum = doc_skirt.select("div.list-box li.li_box");
						Document doc2 = Jsoup.connect("https://www.musinsa.com/app/goods/"+pNum.get(j).attr("data-goods-no")+"?loc=goods_rank").get();
						Elements pName = doc_skirt.select("div.list-box li.li_box div.article_info p.list_info a");
						Elements pBrand = doc_skirt.select("div.list-box li.li_box div.article_info p.item_title a");
						Elements pPrice = doc2.select("ul.product_article #normal_price");
						Elements imgSmall = doc_skirt.select("div.list-box li.li_box img.lazyload");
						Elements imgBig = doc2.select("div.wrap_product div.product-img img");
						
						String pCategory = "스커트";
						...
						File fileSmall = new File("C:\\Works2\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp1\\wtpwebapps\\Hosinsa\\images\\skirt\\small_"+pNumber+".jpg");
						File fileBig = new File("C:\\Works2\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp1\\wtpwebapps\\Hosinsa\\images\\skirt\\big_"+pNumber+".jpg");
						...
						proimgSmall = "skirt\\small_"+pNumber+".jpg";
						proimgBig = "skirt\\big_"+pNumber+".jpg";
